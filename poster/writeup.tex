...
\author{Michael Wang\\\small Math 242 \textbullet{} Duke University \textbullet{} Spring 2026}
\date{}
...
The solver determines that the optimal opening guess is
\texttt{RAISE} and guarantees a solution in exactly $4$ guesses,
matching the published optimum $D(A) = 4$. The initial run takes
approximately $83$ seconds, after which the policy is cached to disk
and later runs are instantaneous.
...
In Evil mode, the deterministic structure of the game permits an exact
solution in pure Python. My solver finds that \texttt{RAISE} is the
optimal opening guess and guarantees a win in $4$ guesses, matching
the published bound $D(A) = 4$. In Unknown mode, the Bayesian
posterior over game type concentrates within two turns, so mode-aware
strategies have only a narrow window in which to outperform
mode-agnostic ones. Empirically, that window is too small to matter
on this corpus: the best Unknown-mode policy is in fact the
mode-agnostic \texttt{expected-entropy}, which beats every mode-aware
blend.
...
\bibitem[Bertsimas \& Paskov(2022)]{bertsimas2022wordle}
Bertsimas, D., \& Paskov, A.~(2022).
\newblock An exact and interpretable solution to Wordle.
\newblock \emph{MIT Sloan Operations Research Center}. Available at
